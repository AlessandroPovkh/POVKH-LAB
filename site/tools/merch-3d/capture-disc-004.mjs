...
const source = JSON.parse(await readFile(path.join(here, "disc-004.source.json"), "utf8"));
const baseUrl = process.env.PVKH_MERCH_QA_URL || "http://127.0.0.1:4173/merch/cd/";
const assetKey = "disc-004";
const profiles = [
...
await sharp(frontFile).extract({left:Math.floor(frontMeta.width*0.14),top:Math.floor(frontMeta.height*0.16),width:Math.floor(frontMeta.width*0.72),height:Math.floor(frontMeta.height*0.56)}).resize({width:1400,kernel:"lanczos3"}).png({compressionLevel:9}).toFile(cropFile);
const cropBytes=await readFile(cropFile);captures.push({view:"desktop-readability-crop",path:relative(cropFile),sha256:sha256(cropBytes),expected:"exact governed ASCII attachment without retyped metadata"});

const references=[{path:source.visualReference.path,sha256:source.visualReference.sha256},...source.visualReference.additional];
const canonicalBuffers=[];
for(const reference of references){const canonicalPath=path.resolve(here,"../../../../data-key-003",reference.path);const canonicalBytes=await readFile(canonicalPath);assert.equal(sha256(canonicalBytes),reference.sha256,"canonical visual reference drift");canonicalBuffers.push(await sharp(canonicalBytes).resize(640,640,{fit:"contain",background:"#090a0c"}).png().toBuffer());}
const compareFile=path.join(outputDir,`${assetKey}-selected-gallery-compare.png`);
const liveFront=await sharp(frontFile).resize(640,640,{fit:"contain",background:"#090a0c"}).png().toBuffer();
const liveRear=await sharp(path.join(outputDir,`${assetKey}-desktop-rear.png`)).resize(640,640,{fit:"contain",background:"#090a0c"}).png().toBuffer();
const comparePanels=[...canonicalBuffers,liveFront,liveRear];
await sharp({create:{width:comparePanels.length*640,height:640,channels:4,background:"#090a0c"}}).composite(comparePanels.map((input,index)=>({input,left:index*640,top:0}))).png({compressionLevel:9}).toFile(compareFile);
const compareBytes=await readFile(compareFile);
const qa={schemaVersion:1,assetKey,baseUrl,capturedAtPolicy:"timestamp-omitted-for-determinism",views:captures,visualComparison:{canonicalReferences:references,artifactPath:relative(compareFile),artifactSha256:sha256(compareBytes),layout:"case / open / material-detail / live-front / live-rear"},checks:{sixRequiredViews:profiles.every(profile=>captures.some(capture=>capture.view===profile.name)),noBrowserErrors:captures.every(capture=>!capture.errors||capture.errors.length===0),readabilityCrop:true,modelLoaded:captures.filter(capture=>capture.loaded!==undefined).every(capture=>capture.loaded)}};
